  ...
  (let (
    ...
    (let (
      ...
      (let (
        ...
        (updated-appraisals (unwrap! (as-max-len? (append current-appraisals {
                               appraiser: appraiser,
                               value: value,
                               timestamp: block-height
                             }) u10) err-invalid-parameter))
      )
        (map-set appraisal-requests
          { request-id: request-id }
          (merge request {
...
(define-private (get-median (values (list 10 uint)))
  (let (
    ...
    (if (is-eq (mod len u2) u0)
      ...
      (let (
        (val1 (unwrap-panic (element-at values middle)))
        (val2 (unwrap-panic (element-at values (- middle u1))))
      )
        (/ (+ val1 val2) u2)
      )
      ;; Odd number of values, take middle one
      (unwrap-panic (element-at values middle))
    )
  )
)
...
(define-private (update-asset-appraisal
  ...
  (let (
    ...
    (new-history (unwrap! (as-max-len? (append appraisal-history {
                           value: value,
                           timestamp: block-height,
                           appraiser: tx-sender
                         }) u10) false))
...
(define-private (calculate-interest-rate
  (collection { contract: principal, base-uri: (string-utf8 256), max-ltv: uint, min-interest-rate: uint, max-interest-rate: uint, interest-rate-model: (string-ascii 10), rarity-levels: (list 10 (string-ascii 20)), min-value: uint, max-value: uint, enabled: bool })
  (asset { current-appraisal: uint, rarity-score: uint, rarity-rank: uint, traits: (list 20 { trait-type: (string-ascii 32), value: (string-utf8 64) }), last-appraisal-date: uint, appraisal-count: uint, appraisal-history: (list 10 { value: uint, timestamp: uint, appraiser: principal }), collection-id: (string-ascii 32) })
  (ltv uint))
  ...
  (let (
    ...
    (if (is-eq model "linear")
      ...
      (let (
        ...
        (final-rate (+ min-rate ltv-component (if (> rarity-adjustment ltv-component) u0 (- ltv-component rarity-adjustment))))
      )
      ...
      (let (
        ...
        (final-rate (+ base-rate (/ (* (- max-rate min-rate) ltv-exponent) u1) (if (> rarity-discount base-rate) u0 (- base-rate rarity-discount))))
      )
        ;; Ensure within bounds
        (if (< final-rate min-rate)
          min-rate
...
(define-private (accrue-interest (loan-id uint))
  (let (
    (loan (unwrap-panic (map-get? loans { loan-id: loan-id })))
    (blocks-elapsed (- block-height (get last-interest-accrual loan)))
    (annual-rate (get interest-rate loan))
    (remaining (get remaining-amount loan))
    ;; Simple interest calculation: principal * rate * time
    (interest-accrued (/ (* remaining annual-rate blocks-elapsed) (* u10000 u52560))) ;; 52560 blocks per year
    (new-remaining (+ remaining interest-accrued))
  )
    (map-set loans
      { loan-id: loan-id }
      (merge loan {
    ...
    (merge loan { remaining-amount: new-remaining })
  )
)
...
(define-public (repay-loan (loan-id uint) (amount uint))
  (let (
    ...
    (let (
      ...
      ;; Update loan state
      (let (
        (new-remaining (- remaining repay-amount))
        (new-repaid (+ (get repaid-amount updated-loan) repay-amount))
        (is-fully-repaid (is-eq new-remaining u0))
      )
        (map-set loans
          { loan-id: loan-id }
          (merge updated-loan {
            ...
            remaining-amount: new-remaining,
            state: (if is-fully-repaid u1 u0) ;; 1=Repaid, 0=Active
          })
        )
        
        ;; If fully repaid, release collateral
        (if is-fully-repaid
          (release-collateral loan-id)
          true
        )
        
        ;; Update borrower history
        (update-borrower-repayment borrower repay-amount is-fully-repaid)
        
        (ok {
          loan-id: loan-id,
          amount-repaid: repay-amount,
          remaining-amount: new-remaining,
          fully-repaid: is-fully-repaid
        })
      )
    )
  )
...
(define-private (release-collateral (loan-id uint))
  (let (
    (collateral (unwrap-panic (map-get? locked-collateral { loan-id: loan-id })))
    (owner (get owner collateral))
  )
    ;; Remove from locked collateral
    (map-delete locked-collateral { loan-id: loan-id })
    
    ;; In a real implementation, transfer NFT back to owner
    ;; For simplicity, we just return true
    true
  )
)

;; Update borrower history on repayment
(define-private (update-borrower-repayment (borrower principal) (amount uint) (is-full bool))
  (let (
    (existing (unwrap-panic (map-get? borrower-history { borrower: borrower })))
  )
    (map-set borrower-history
      { borrower: borrower }
      (merge existing {
        active-loans: (if is-full (- (get active-loans existing) u1) (get active-loans existing)),
        repaid-loans: (if is-full (+ (get repaid-loans existing) u1) (get repaid-loans existing)),
        current-debt: (- (get current-debt existing) amount),
        last-activity: block-height
      })
    )
...
(define-read-only (get-collection (collection-id (string-ascii 32)))
  (map-get? collections { collection-id: collection-id })
)

;; Get NFT asset details
(define-read-only (get-nft-asset (collection-id (string-ascii 32)) (token-id uint))
  (map-get? nft-assets { collection-id: collection-id, token-id: token-id })
)
...
(define-read-only (get-protocol-params)
  {
    ...
    min-loan-duration: (var-get min-loan-duration),
    max-loan-duration: (var-get max-loan-duration),
    auction-duration: (var-get auction-duration),
    grace-period: (var-get grace-period),
    oracle-threshold: (var-get oracle-consensus-threshold),
